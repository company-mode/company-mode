...
* Add buffer-local variable `company-backend-counts` that stores the
  number of completions generated by each backend on last completion
  in each buffer.
* Add `company-echo-completion-counts-frontend` which dynamically echos
  to the minibuffer the number of completions generated by each backend.
