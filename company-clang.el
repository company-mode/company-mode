...
(defun company-clang--parse-error ()
  (goto-char (point-min))
  (if (search-forward company-clang--completion-pattern nil t)
...
(defun company-clang--call-process (prefix &rest args)
  (with-temp-buffer
    (let ((res (apply 'call-process company-clang-executable nil t nil
                      args)))
      (or (company-clang--parse-output prefix)
          (unless (eq 0 res)
            (message "Company-clang command failed with error %d:\n%s" res
                     (mapconcat 'identity args " "))
            (message "%s" (company-clang--parse-error))
            nil)))))
